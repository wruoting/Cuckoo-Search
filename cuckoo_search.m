...
Lb_lower=0;
Lb_higher=pi;
Lb=Lb_lower*ones(1,number_of_dimensions);
% Upper bounds
Ub=Lb_higher*ones(1,number_of_dimensions);
...
N_iter=0;
fcumulative=Lb_lower+(Lb_higher-Lb_lower).*rand(1);
%% Starting iterations
while (abs(fmin-mean(fcumulative))>Tol),
    ...
    if (length(fcumulative) <=50)
        fcumulative=[fcumulative,fmin];
    else
        fcumulative = fcumulative(2:end);
    end
end %% End of iterations
[srt, idxSrt]  = sort(bestnest);
...
function [rosenbrock] = rosenbrock(u)
    % split u up since we are taking different indices
    total=0;
    for n=1:length(u)-1
        total=total + ((1-u(n)).^2 + 100 * (u(n+1)-u(n).^2).^2);
    end
    rosenbrock=total;
    
function [matyas]=matyas(u)
    total=0;
    for n=1:length(u)-1
        total=total+0.26*(u(n).^2+u(n+1).^2)-0.48*u(n)*u(n+1);
    end
    matyas=total;
    
function [michalewicz] = michalewicz(u)
    m=10;
    total=0;
    for n=1:length(u)
        total=total+(sin(u(n))*(sin((n*u(n).^2)/pi)).^(2*m));
    end
    michalewicz=-total;
